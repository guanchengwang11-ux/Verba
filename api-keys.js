window.VerbaI18n.ready.then(() => {
  ...
  const inputs = { openai: document.querySelector("#openaiKey"), gemini: document.querySelector("#geminiKey"), deepseek: document.querySelector("#deepseekKey"), groq: document.querySelector("#groqKey") };
  ...
  document.querySelector('#deleteGroqKey').addEventListener('click', async () => {
    try { const response = await fetch('/api/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ keys: { groq: null } }) });
      if (!response.ok) throw new Error(i18n.error('UNKNOWN'));
      inputs.groq.value = ''; showStatus((await response.json()).keys); message.textContent = i18n.t('api.groqKeyDeleted');
