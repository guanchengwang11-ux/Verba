...
test("detects names, multi-word names, brands, acronyms, accounts, and identifiers", () => {
  const text = "Muyiwa told Lin Zhang and Wang Guancheng to check Bybit Binance OKX KCEX OpenAI Google DeepSeek Ajuba UID KYC KYB AML API P&L VIP SOP FAQ @David @nana123 user@example.com https://example.com CASE-20260907 987654";
...
test("excludes common sentence starters, weekdays, and language names", () => {
  const entities = detectEntities("Please check the report. Hello David. English is used on Monday.").map((entity) => entity.value);
  assert.ok(entities.includes("David"));
  for (const value of ["Please", "Hello", "English", "Monday"]) assert.ok(!entities.includes(value), `did not expect ${value}`);
});

test("restores exact and mildly altered placeholder formats", () => {
  const { entityMap } = protectEntities("Nana, David, Bybit, UID");
...
test("builds instructions without relying on the model to identify entities", () => {
  const { entityMap } = protectEntities("Nana and UID");
  ...
  assert.doesNotMatch(instruction, /Nana|UID/);
});
